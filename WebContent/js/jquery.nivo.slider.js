...
if(vars.currentImage.attr('title')!=''){var title=vars.currentImage.attr('title');if(title.substr(0,1)=='#')title=$(title).html();if($('.nivo-caption',slider).css('display')=='block'){$('.nivo-caption',slider).animate({right:'-500'}, 600, 'easeInBack').fadeOut(settings.animSpeed,function(){$(this).html(title);$(this).animate({right:'0'}, 600, 'easeOutBack', function(){$(this).find('as').animate({marginRight:'20'},1200,'easeOutElastic')}).find('as').animate({marginRight:'-150'},400,'easeOutBack').parent().fadeIn(settings.animSpeed); settings.customChange.call(this);});}else{$('.nivo-caption',slider).html(title);} settings.customChange.call(this);
